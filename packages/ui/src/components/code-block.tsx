...
function normalizeLanguage(lang?: string): Language {
  ...
  return (supported as unknown as string[]).includes(cleaned)
    ? (cleaned as Language)
    : fallback;
}
...
function CodeBlock({
  ...
  return (
    <div
      className={cn(
        "w-full overflow-hidden border bg-card text-foreground",
        className
      )}
    >
      {(filename || language) && (
        <div className="flex items-center justify-between border-b bg-secondary px-3 py-2">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <span className="ml-2 rounded-md bg-muted px-2 py-0.5 text-muted-foreground text-xs">
              {lang}
            </span>
          </div>
        </div>
